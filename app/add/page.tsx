  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        console.log('Setting provinces:', loaded)
        setProvinces(loaded ?? [])
      } catch (e) {
  ...
  const fetchCitiesByProvince = async (provinceId: string) => {
    try {
      ...
      if (!loaded) {
        ...
        const data = await res.json()
        console.log(`Cities API response for ${provinceId}:`, data)
        loaded = Array.isArray(data) ? data : data?.cities ?? []
        try { sessionStorage.setItem(cacheKey, JSON.stringify({ ts: now, data: loaded })) } catch {}
      }
      console.log(`Setting cities for ${provinceId}:`, loaded)
      setCityOptions(loaded ?? [])
    } catch (e) {
      console.error(`Error loading cities for ${provinceId}:`, e)
      setCityOptions([])
    } finally {
