  ...
  const handleSubmitReview = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast({ title: `Error (${response.status})`, description: message, variant: "destructive" })
      }
    } catch (e) {
          ...
          <Dialog open={openReview} onOpenChange={setOpenReview}>
            <DialogContent>
              ...
              <DialogFooter>
                ...
                <DialogClose asChild>
                  <Button onClick={handleSubmitReview} disabled={submitting || reviewComment.trim().length < 3}>Submit</Button>
                </DialogClose>
              </DialogFooter>
            </DialogContent>
          </Dialog>
