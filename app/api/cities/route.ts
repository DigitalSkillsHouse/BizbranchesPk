...
export async function GET(request: NextRequest) {
  ...
  if (provinceId) {
    ...
    const response = NextResponse.json({ ok: true, cities });
  ...
  if (!base || !apiKey || !apiPassword) {
    ...
    const response = NextResponse.json({ ok: true, cities: fallback });
    response.headers.set('Cache-Control', 's-maxage=3600, stale-while-revalidate=86400');
    return response;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const fallback = getAllCities();
      const res = NextResponse.json({ ok: true, cities: fallback });
    ...
    const res = NextResponse.json({ ok: true, cities });
